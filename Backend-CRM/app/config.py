...
class Settings(BaseSettings):
    ...
    backend_internal_url: str = "http://backend:8000"  # For ONLYOFFICE container to reach local backend
    
    @property
    def is_production(self) -> bool:
        """Check if running in production environment"""
        return "azurewebsites.net" in str(self.backend_internal_url)
    
    @property 
    def effective_backend_url(self) -> str:
        """Get the correct backend URL for the current environment"""
        # if self.is_production:
        #     return self.backend_internal_url
