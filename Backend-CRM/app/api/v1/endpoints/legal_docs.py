...
def extract_content_controls(docx_bytes: bytes) -> dict:
    ...
    try:
        ...
        logger.error(f"Failed to extract DOCX content controls: {e}")
        return {}
...
async def create_agreement_document_from_template(
    ...
    # Check if template file exists (handle both local and Azure URLs)
    template_file_path = template.template_file_path
    if template_file_path.startswith('http'):
        # Azure URL - verify blob exists
        from app.azure_storage import azure_storage
        try:
            # Extract blob name from URL
            blob_name = template_file_path.split("/")[-1]
            client = azure_storage.get_client()
            if not client:
                raise HTTPException(status_code=500, detail="Azure storage not available")
            
            container_client = client.get_container_client(azure_storage.container_name)
            blob_client = container_client.get_blob_client(blob_name)
            blob_properties = blob_client.get_blob_properties()
            print(f"✅ Verified Azure blob exists: {blob_name}")
        except Exception as e:
            print(f"❌ Azure blob not found: {template_file_path}")
            raise HTTPException(
                status_code=404,
                detail=f"Template DOCX file not found"
            )
    else:
        # Local file path - check if file exists
        template_docx_path = Path(template_file_path)
        if not template_docx_path.exists():
            raise HTTPException(
                status_code=404,
                detail=f"Template DOCX file not found:"
    ...
    # Handle template path for placeholder replacement (Azure vs local)
    if template_file_path.startswith('http'):
        # Azure URL - download to temporary file first
        import tempfile
        from app.azure_storage import azure_storage
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix='.docx', delete=False) as temp_file:
                ...
                template_docx_path = Path(temp_file.name)
                
            print(f"✅ Downloaded Azure template to temp file: {template_docx_path}")
        except Exception as e:
            print(f"❌ Failed to download Azure template: {e}")
            raise HTTPException(status_code=500, detail="Failed to download template from Azure")
    else:
        # Local file path
        template_docx_path = Path(template_file_path)
    ...
    try:
        ...
        logger.info(f"✅ Placeholder replacement completed successfully")
    except Exception as e:
        logger.error(f"❌ Failed to replace placeholders in document: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
    ...
    if template_file_path.startswith('http') and template_docx_path.exists():
        template_docx_path.unlink()
        print(f"🧹 Cleaned up temporary file: {template_docx_path}")
...
async def save_agreement_document(
    ...
    """
    Frontend trigger for saving. 
    DO NOT create a DB record here. ONLYOFFICE callback handles the actual save.
    """
    # Just verify permissions and return the current latest document
    agreement_result = await db.execute(
        select(Agreement)
        .where(Agreement.id == agreement_id)
        .options(selectinload(Agreement.documents)) 
    ...
    if not permissions["can_save"]:
        raise HTTPException(
            status_code=403,
            detail=f"Cannot save document. Agreement is locked."
        )

    # Return the existing latest document. 
    # The ONLYOFFICE background callback will handle creating the NEW version.
    latest_document = max(agreement.documents, key=lambda d: d.version_number)
    
    return {
        "id": latest_document.id,
        "agreement_id": latest_document.agreement_id,
        "version_number": latest_document.version_number,
        "document_content": latest_document.document_content if latest_document.document_content else {},
        "document_file_path": latest_document.document_file_path if hasattr(latest_document, 'document_file_path') else None,
        "created_from_template_id": latest_document.created_from_template_id,
        "created_by": latest_document.created_by,
        "created_at": latest_document.created_at,
        "is_signed_version": latest_document.is_signed_version,
    }
    db.add(new_document)
    await db.flush()
...
#             logger.error(f"Callback failed: Agreement {agreement_id} or documents not found.")
#             from fastapi.responses import JSONResponse
#             return JSONResponse(
#                 content={"error": 1},
#                 headers={
...
#         # Status 2: Document is being saved (CREATE version for content changes)
#         # Status 6: Document is being force saved (CREATE version for manual save)
#         # Status 7: Document save error (do NOT create version)
        
#         # Create version for both status 2 (regular save) and status 6 (force save)
#         if status in [2, 6]:
...
#                 # logger.info(
#                 #     "ONLYOFFICE callback status %s (document saved) for agreement %s - creating new version",
#                 #     status,
#                 #     agreement_id
#                 # )

#                 # Compute hash of latest version file and read its content
#                 latest_file_hash = None
...
#                         latest_file_hash = hashlib.sha256(latest_file_content).hexdigest()
#                     except Exception as hash_error:
#                         logger.warning(f"Failed to process latest version file: {hash_error}")

#                 # If hashes are identical, skip version creation
#                 if latest_file_hash and new_file_hash == latest_file_hash:
#                     return JSONResponse(content={"error": 0}, headers={"Access-Control-Allow-Origin": "*"})

#                 # --- NEW CRASH-PROOF DIFFING LOGIC ---
#                 changes_summary = []
#                 try:
#                     if latest_file_content:
#                         old_controls = extract_content_controls(latest_file_content)
#                         new_controls = extract_content_controls(docx_content)

#                         # Compare old and new fields
#                         for field, new_val in new_controls.items():
#                             old_val = old_controls.get(field)
#                             if old_val is not None and old_val != new_val:
#                                 changes_summary.append(f"• {field}: '{old_val}' → '{new_val}'")
#                 except Exception as diff_err:
#                     logger.error(f"Error during document diffing: {diff_err}")
#                 # -------------------------------------
...
#                 # --- CRASH-PROOF COMMENT CREATION ---
#                 save_type = "manual save" if status == 6 else "auto save"
                
#                 # Safely parse users (prevents crashes if ONLYOFFICE sends integers instead of strings)
#                 try:
#                     users = body.get("users", [])
#                     user_text = f" by user ID(s): {', '.join(str(u) for u in users)}" if users else ""
#                 except Exception:
#                     user_text = ""
                
#                 base_message = f"Version {next_version_number} created after document {save_type}{user_text}."
                
#                 if changes_summary:
#                     detailed_message = f"{base_message}\n\nUpdates:\n" + "\n".join(changes_summary)
#                 else:
#                     detailed_message = f"{base_message}\n\n(General text or formatting was modified)"

#                 # Link comment to 'None' safely
#                 await create_agreement_system_comment(
#                     db,
#                     agreement.id,
#                     None, 
...
async def onlyoffice_callback(
    ...
    from sqlalchemy.orm import selectinload
    from app.azure_storage import azure_storage 
    from app.models import Agreement, AgreementDocument, AgreementComment, CommentType
    ...
    res = await db.execute(select(Agreement).where(Agreement.id == agreement_id).options(selectinload(Agreement.documents)))
    ...
    try:
        ...
        download_url = urlunparse(parsed._replace(netloc="backend-crm-onlyoffice-1:80"))
    except Exception:
        pass
    ...
    async with httpx.AsyncClient(verify=False) as client:
        ...
        if resp.status_code != 200:
            return {"error": 1}
        docx_content = resp.content

    # --- VERSION DEBOUNCE LOGIC ---
    latest_doc = max(agreement.documents, key=lambda d: d.version_number)
    
    now = dt_module.datetime.now(dt_module.timezone.utc)
    # Ensure created_at has timezone info
    last_created = latest_doc.created_at
    if last_created.tzinfo is None:
        last_created = last_created.replace(tzinfo=dt_module.timezone.utc)

    # If the last version was created less than 30 seconds ago, 
    # we update the EXISTING version instead of making a new one.
    is_duplicate_request = (now - last_created) < dt_module.timedelta(seconds=30)

    if is_duplicate_request:
        target_version = latest_doc.version_number
        # Reuse the existing path to overwrite the file on Azure
        cloud_filename = latest_doc.document_file_path.split('/')[-1]
        if "agreements/" not in cloud_filename:
             cloud_filename = f"agreements/{agreement.id}/{cloud_filename}"
    else:
        target_version = latest_doc.version_number + 1
        cloud_filename = f"agreements/{agreement.id}/version_{target_version}_{uuid.uuid4()}.docx"

    # Azure Upload (Overwrites if cloud_filename is the same)
    docx_stream = io.BytesIO(docx_content)
    azure_url = await azure_storage.upload_file(
        file_data=docx_stream, 
        blob_name=cloud_filename,
    ...
    if not is_duplicate_request:
        # Create NEW database record ONLY if it's not a rapid-fire duplicate
        new_doc = AgreementDocument(
            agreement_id=agreement.id,
            version_number=target_version,
            document_file_path=azure_url,
            document_html='',
            created_by=latest_doc.created_by,
            is_signed_version='false',
        )
        db.add(new_doc)
        
        timeline_log = AgreementComment(
            agreement_id=agreement.id,
            created_by=latest_doc.created_by, 
            comment_type=CommentType.SYSTEM,
            content=f"Document updated to version {target_version}."
        )
        db.add(timeline_log)
    else:
        # Just update the timestamp on the current record
        latest_doc.created_at = now
...
async def get_onlyoffice_config(
    ...
    # Find the absolute latest version number for this agreement
    latest_version_number = max(d.version_number for d in agreement.documents)

    # Determine edit mode based on agreement status AND version history
    # Locking rules:
    # - EXECUTED: Disable editor entirely (return error)
    ...
    # Force VIEW mode under the following conditions:
    # 1. The agreement is out for signature
    # 2. The user lacks edit permissions
    # 3. The requested document is an OLDER version (not the latest)
    if agreement.status == AgreementStatusEnum.SENT_FOR_SIGNATURE:
        mode = "view"
    elif document.version_number < latest_version_number:
        mode = "view"  # <-- Locks older versions natively in ONLYOFFICE
    else:
    ...
    # Use IP address instead of container name for reliable communication
    internal_base = "http://172.18.0.6:8000"
...
async def get_agreement_document_file(
    ...
    # 1. Fetch Agreement and Documents
    agreement_result = await db.execute(
        select(Agreement)
        .where(Agreement.id == agreement_id)
        .options(selectinload(Agreement.documents))
    ...
    if not agreement or not agreement.documents:
        raise HTTPException(status_code=404, detail="Agreement or documents not found")
    ...
    if not document or not document.document_file_path:
        raise HTTPException(status_code=404, detail="Document file path missing")
    
    file_path = document.document_file_path
    
    # Define common headers for ONLYOFFICE
    headers = {
        "Access-Control-Allow-Origin": "*",
    ...
    # 3. Handle Azure Blob Storage
    if file_path.startswith("https://") and ".blob.core.windows.net/" in file_path:
        try:
            # FIX: Extract the FULL blob path (e.g., agreements/uuid/file.docx)
            # This splits the URL and takes everything after the container name
            container_name = settings.azure_storage_container_name
            blob_name = file_path.split(f"/{container_name}/")[-1]
    ...
    # 4. Handle Local File
        ...
        return FileResponse(
            path=str(local_path),
            media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            filename=f"agreement_v{document.version_number}.docx",
            headers=headers
        )
...
def generate_template_blob_name(study_name: str, site_name: str, template_type: str, template_name: str) -> str:
    """
    Generate Azure blob name with pattern: studyName_siteName_templateType_templateName.docx
    
    Args:
        study_name: Study name
        site_name: Site name  
        template_type: Template type (CDA, CTA, etc.)
    ...
    def clean_component(component: str) -> str:
        ...
        cleaned = re.sub(r'[^\w\s-]', '', component)  # Remove special chars except hyphens and underscores
        cleaned = re.sub(r'[-\s]+', '_', cleaned)     # Replace hyphens and spaces with underscores
        return cleaned.strip('_')[:50]               # Limit to 50 chars and trim
    
    study_clean = clean_component(study_name)
    site_clean = clean_component(site_name)
    type_clean = clean_component(template_type)
    template_clean = clean_component(template_name)
    
    # Generate the final blob name with clear separators
    blob_name = f"{study_clean}__{site_clean}__{type_clean}__{template_clean}.docx"
...
async def create_study_template(
    ...
    try:
        ...
        # Get site information for naming pattern
        logger.info("🔍 Getting site information for naming pattern...")
        site_name = "UnknownSite"  # Default fallback
        
        try:
            # Get the first site associated with this study
            study_site_result = await db.execute(
                select(StudySite, Site)
                .join(Site, StudySite.site_id == Site.id)
                .where(StudySite.study_id == study_id)
                .limit(1)
            )
            study_site_data = study_site_result.first()
            
            if study_site_data:
                site = study_site_data.Site
                site_name = site.name or site.site_id or "UnknownSite"
                logger.info(f"✅ Found site: {site_name}")
            else:
                logger.warning(f"⚠️ No sites found for study {study_id}, using default site name")
        except Exception as e:
            logger.warning(f"⚠️ Error getting site information: {e}, using default site name")
        
        # Generate filename with new pattern: studyName_siteName_templateType_templateName.docx
        logger.info("🔍 Generating filename with new pattern...")
        file_name = generate_template_blob_name(
            study_name=study.name,
            site_name=site_name,
        ...
        try:
            # Try to import azure storage (may not be available)
            from app.azure_storage import azure_storage
            ...
            # Azure storage not available, use local storage
            logger.info(f" Azure storage not available ({e}), using local storage...")
        ...
        doc_content = None
        placeholder_config = {}  # Default to empty
        
        # Detect placeholders from the DOCX file
        try:
            from app.utils.docx_placeholder_replace import _extract_placeholders_from_docx
            from docx import Document
            ...
            if azure_blob_url:
                ...
                try:
                    from app.azure_storage import azure_storage
                    logger.info(f" Downloading template from Azure for placeholder detection: {file_name}")
                    file_data = await azure_storage.download_file(file_name)
                    
                    if file_data:
                        # Create Document object from downloaded data
                        import io
                        ...
                        logger.info(f" Found {len(placeholders_found)} placeholders: {placeholders_found}")
                        
                        # Create placeholder config with all placeholders editable by default
                        placeholder_config = {placeholder: {"editable": True} for placeholder in placeholders_found}
                        
                        logger.info(f" Created placeholder config: {placeholder_config}")
                    else:
                        logger.warning(f" Could not download template from Azure for placeholder detection")
                except ImportError as e:
                    logger.warning(f"Azure storage not available for placeholder detection: {e}")
                except Exception as e:
                    logger.error(f"Error downloading from Azure for placeholder detection: {e}")
            else:
                logger.info(f" Processing local template file for placeholder detection: {template_file_path}")
                # Create Document object from local file
            ...
            # Continue with empty placeholder config
...
async def get_template_field_mapping_options():
    ...
    return {
        "site_profile": [
            ...
            {"field": "full_address", "label": "Full Address"},
...
async def get_template_onlyoffice_config(
    ...
    file_url = f"{settings.effective_backend_url}/api/templates/{template_id}/template-file"
    ...
    config = {
        ...
        "editorConfig": {
            "mode": "view",
            "lang": "en",
            "callbackUrl": f"{settings.effective_backend_url}/api/templates/{template_id}/onlyoffice-callback",
            "user": {
...
async def get_template_file(
    ...
    if file_path.startswith("https://") and ".blob.core.windows.net/" in file_path:
        ...
        try:
            ...
            logger.error(f"Azure storage not available for template download: {e}")
            raise HTTPException(
                status_code=500,
                detail="Azure storage not available for template download"
            )
        except Exception as e:
            logger.error(f"Error downloading from Azure: {e}")
            raise HTTPException(
                status_code=500,
                detail=f"Error downloading template from Azure: {str(e)}"
            )
            headers = {
                "Access-Control-Allow-Origin": "*",
                "Access-Control-Allow-Methods": "GET, OPTIONS",
