...
def get_placeholder_value_from_mapping(
    ...
    Returns:
        ...
        Falls back to legacy SiteProfile mapping if explicit field_mappings
        are not provided for this placeholder.
    """
    mapping_path: Optional[str] = None

    # Preferred path: explicit field_mappings from template configuration
    if field_mappings:
        mapping_path = field_mappings.get(placeholder_name.upper())
        if not mapping_path:
            logger.debug(
                "No explicit field_mappings entry for placeholder '%s'; "
                "will try legacy mapping fallback.",
                placeholder_name,
            )

    # Fallback path: use LEGACY_PLACEHOLDER_MAP (site_profile.*) when no mapping
    if not mapping_path:
        legacy_field = LEGACY_PLACEHOLDER_MAP.get(placeholder_name.upper())
        if legacy_field:
            mapping_path = f"site_profile.{legacy_field}"
            logger.debug(
                "Using legacy mapping for placeholder '%s' -> %s",
                placeholder_name,
                mapping_path,
            )
        else:
            logger.warning(
                "No field_mappings or legacy mapping found for placeholder: %s",
                placeholder_name,
            )
            return None
...
def replace_placeholders_in_docx(
    ...
    try:
        ...
        if use_dynamic_mapping:
            ...
            # Replace placeholders using dynamic mappings
            replacements_made = 0
            ...
            for placeholder_name in placeholders_found:
                ...
                # If value is not None, we replace the placeholder.
                # This allows empty strings / "0" / "False" to be inserted,
                # and keeps the placeholder only when the underlying field is null.
                if value is not None:
                    if is_editable:
                        # Editable fields: Replace with unlocked content control (editable region)
                        logger.info(f"Replacing editable placeholder {placeholder_name} with content control (editable)")
                        _replace_with_editable_content_control(
                            doc, placeholder_text, value, placeholder_name
                        )
                        replacements_made += 1
                    else:
                        # Locked fields: Replace as plain text (document will be read-only except content controls)
                    ...
                    logger.debug(f"Replaced {placeholder_name} with value: {value} (editable={is_editable})")
                else:
            ...
            logger.info(f"Placeholder replacement complete: {replacements_made} replacements made, {replacements_skipped} skipped")
        else:
...
def _replace_signature_blocks_in_docx(
    ...
    for paragraph in doc.paragraphs:
        if "{{SITE_SIGNATURE_BLOCK}}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{{SITE_SIGNATURE_BLOCK}}", site_sig_text)
        if "{{SPONSOR_SIGNATURE_BLOCK}}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{{SPONSOR_SIGNATURE_BLOCK}}", sponsor_sig_text)
