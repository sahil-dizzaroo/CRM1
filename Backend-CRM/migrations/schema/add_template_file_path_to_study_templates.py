"""
Migration script to add template_file_path column to study_templates table.

This allows storing DOCX files for templates in ONLYOFFICE integration.
"""
...
async def add_template_file_path_column():
    """Add template_file_path column to study_templates table."""
    print("=" * 70)
    print("Add template_file_path to StudyTemplate Table Migration")
    print("=" * 70)
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            if not column_exists:
                print("  Adding 'template_file_path' column to 'study_templates' table...")
                await session.execute(text("""
                    ALTER TABLE study_templates
                    ADD COLUMN template_file_path TEXT;
                """))
                print("  ✅ 'template_file_path' column added successfully.")
            else:
                print("  ⚠️ 'template_file_path' column already exists. Skipping.")
...
if __name__ == "__main__":
    try:
        asyncio.run(add_template_file_path_column())
